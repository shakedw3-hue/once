# ורדה — עוזרת קולית אישית

> "היי ורדה" — שליטה מלאה במחשב, באפליקציות, ובחיים — דרך הקול.

---

## מה זה

אפליקציית דסקטופ שרצה תמיד ברקע על המק. מזהה את מילת ההפעלה "היי ורדה", מקליטה פקודה קולית בעברית, מבינה מה צריך לעשות, מבצעת, ועונה בקול + טקסט על המסך.

---

## חוויית משתמש

1. המק נדלק — ורדה עולה אוטומטית
2. אייקון סגול קטן ב-menubar (ליד השעון)
3. המשתמש אומר **"היי ורדה"** — האייקון נדלק, ורדה מקשיבה
...
5. ורדה מבצעת את הפעולה
6. ורדה עונה בקול (TTS) + מציגה טקסט בחלון קטן על המסך
7. החלון נעלם אחרי כמה שניות — ורדה חוזרת להאזנה
...
| מקשיבה (אחרי "היי ורדה") | סגול בוהק, פועם | "מקשיבה..." |
...
| Wake word listener | Python + Porcupine (Picovoice) | זיהוי "היי ורדה" — רץ מקומי, בלי אינטרנט |
...
- **העתקה ללוח:** `pbcopy` — ורדה יכולה להעתיק תוצאות ל-clipboard
...
הודעות (מייל, WhatsApp) דורשות אישור. ורדה אומרת:
> "אני עומדת לשלוח מייל לגולן: 'הפגישה מחר ב-3. נתראה.' — לשלוח?"

המשתמש אומר "כן" / "שלחי" → ורדה שולחת.
המשתמש אומר "לא" / "בטלי" → ורדה מבטלת.

---

## Claude Memory — רעיונות ומחשבות

כשהמשתמש אומר "יש לי רעיון" / "תקליטי" / "תזכרי ש...", ורדה:
...
ורדה צריכה לדעת מי זה "גולן", "אבי" וכו'. מקורות:

1. **Google Contacts API** — סנכרון אנשי קשר (שם, מייל, טלפון)
2. **למידה מתמשכת** — אם המשתמש אומר "תשלחי לגולן" ואין גולן באנשי קשר, ורדה שואלת: "מי זה גולן? מה המייל שלו?" ושומרת
...
2. ורדה מבקשת הרשאת מיקרופון (macOS permission)
3. ורדה מבקשת הרשאת Accessibility (לשליטה באפליקציות)
...
- Wake word באנגלית בלבד ("Hey Varda") — Porcupine לא תומך בעברית ל-custom wake words
